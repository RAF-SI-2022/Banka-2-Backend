...
import rs.edu.raf.si.bank2.Bank2Backend.bootstrap.BootstrapData;
import rs.edu.raf.si.bank2.Bank2Backend.models.mariadb.Forex;
import rs.edu.raf.si.bank2.Bank2Backend.repositories.mariadb.ForexRepository;
import rs.edu.raf.si.bank2.Bank2Backend.services.interfaces.ForexServiceInterface;
...
public class ForexService implements ForexServiceInterface {
    ...
    public Forex getForexForCurrencies(String fromCurrency, String toCurrency) {
        ...
        if (forex.isPresent()) {
            ...
            try {
                Date lastModifiedDate = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")
                        .parse(forex.get().getLastRefreshed());
                Date currentDateTime = dateFormat.parse(systemTime);
                long timeDiff = currentDateTime.getTime()

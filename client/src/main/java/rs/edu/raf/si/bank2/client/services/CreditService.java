...
public class CreditService {
    ...
    public CreditService(CreditRepository creditRepository, CreditRequestRepository creditRequestRepository,
                         PayedInterestRepository payedInterestRepository, TekuciRacunRepository tekuciRacunRepository, DevizniRacunRepository devizniRacunRepository,
                         PoslovniRacunRepository poslovniRacunRepository, RacunStorageRepository racunStorageRepository) {
    ...
    public CreditRequest saveRequest(CreditRequestDto dto) {
        return creditRequestRepository.save(new CreditRequest(
                dto.getClientEmail(), CreditApproval.WAITING, dto.getAmount(), dto.getUsedFor(),
                dto.getMonthlyRate(), dto.getClientHasJob(), dto.getJobLocation(), dto.getCurrentJobDuration(),
                dto.getDueDateInMonths(), dto.getPhoneNumber())
        );
    }
    ...
    public CommunicationDto approveCredit(String creditId, CreditDto dto) {
        ...
        Credit credit = new Credit(
                dto.getClientEmail(), dto.getName(), dto.getAccountRegNumber(), dto.getAmount(),
                dto.getAmount(), dto.getRatePercentage(), monthlyRate, //todo porveri dal je ok
                dtf.format(LocalDate.now()), dto.getDueDate(), dto.getCurrency());
    ...
    private void increaseOrDecreaseUserBalance(Double amount, String racunRegNum, boolean decrease){
        Optional<RacunStorage> racunInfo = racunStorageRepository.findRacunStorageByBalanceRegistrationNumber(racunRegNum);

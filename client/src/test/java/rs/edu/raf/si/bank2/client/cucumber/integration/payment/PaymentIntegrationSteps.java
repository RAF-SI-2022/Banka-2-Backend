...
//    private String paymentDescription;
    ...
    public void make_payment() throws JsonProcessingException {
        PaymentDto paymentDto = new PaymentDto("asdfasf", "13212331", "name", 1.0,
                "2312321", "12321", "asdfasdf");
        String body = new io.cucumber.core.internal.com.fasterxml.jackson.databind.ObjectMapper().writeValueAsString(paymentDto);
        try {
            MvcResult mvcResult = mockMvc.perform(
                    post("/api/payment/removeMoney")
                            .contentType("application/json")
                            .content(body)
    ...
    public void transfer_money() throws JsonProcessingException {
        ...
        try {
            MvcResult mvcResult = mockMvc.perform(
                            post("/api/payment/transferMoney")
                                    .contentType("application/json")
                                    .content(body)
                                    .header("Content-Type", "application/json")
                                    .header("Access-Control-Allow-Origin", "*")
    ...
    public void exchange_money() throws JsonProcessingException {
        ...
        try {
            MvcResult mvcResult = mockMvc.perform(
                            post("/api/payment/exchangeMoney")
                                    .contentType("application/json")
                                    .content(body)
                                    .header("Content-Type", "application/json")
                                    .header("Access-Control-Allow-Origin", "*")
    ...
    public void add_payment_receiver() throws JsonProcessingException {
        PaymentReceiverDto creditRequestDto = new PaymentReceiverDto("test@gmail.com", "name", "13232",
                "12312", "12312", "asdfasdf");
    ...
    public void edit_receiver() throws JsonProcessingException {
        ...
        if (receivers.size() != 0) {
            ...
            try {
                ...
                MvcResult mvcResult = mockMvc.perform(
                        patch("/api/payment/editReceiver/" + receivers.get(0).getId())
                                .contentType("application/json")
                                .content(body)
                                .header("Content-Type", "application/json")
                                .header("Access-Control-Allow-Origin", "*")
    ...
    public void delete_receiver() {
        ...
        if (receivers.size() != 0) {
            try {
                MvcResult mvcResult = mockMvc.perform(
                        delete("/api/payment/deleteReceivers/ " + receivers.get(0).getId())
                                .header("Content-Type", "application/json")
                                .header("Access-Control-Allow-Origin", "*")
                                .header("Authorization", "Bearer " + token))
                        .andExpect(status().isOk())

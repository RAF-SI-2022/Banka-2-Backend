    ...
    public ResponseEntity<?> buyOrSell(@RequestBody @Valid BuySellForexDto dto) {
        ...
        try {
            ...
            return ResponseEntity.badRequest()
                    .body("User with email "+ signedInUserEmail+ ", doesn't have balance in currency "+ forex.getFromCurrencyName());
        }
        catch (NotEnoughMoneyException e2) {
            return ResponseEntity.badRequest()
                    .body("User with email "+ signedInUserEmail+ ", doesn't have enough money in currency "+ forex.getFromCurrencyName()+ " for buying "+ dto.getAmountOfMoney()+ " "+ dto.getToCurrencyCode()+ "("+ forex.getToCurrencyName()+ ")");
        }
        catch (Exception e3) {

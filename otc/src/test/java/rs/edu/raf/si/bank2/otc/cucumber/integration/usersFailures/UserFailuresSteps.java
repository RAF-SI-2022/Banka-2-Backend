package rs.edu.raf.si.bank2.otc.cucumber.integration.usersFailures;
...
public class UserFailuresSteps extends UsersFailureIntegrationTestConfig {
    ...
    public void user_doesnt_exist_in_database() {
        try {
            ...
            String expectedMessage =
                    "Request processing failed; nested exception is rs.edu.raf.si.bank2.exceptions.otc.UserNotFoundException: User with id <-1> not found.";
            String actualMessage = exception.getMessage();
            assertEquals(expectedMessage, actualMessage);
        } catch (Exception e) {
            fail(e.getMessage());
    ...
    public void update_nonexistent_user_in_database() {
        try {
            ...
            String expectedMessage =
                    "Request processing failed; nested exception is rs.edu.raf.si.bank2.otc.exceptions.UserNotFoundException: User with id <-1> not found.";
            String actualMessage = exception.getMessage();
            assertEquals(expectedMessage, actualMessage);
        } catch (Exception e) {
            fail(e.getMessage());
        ...
        try {
            ...
            String expectedMessage =
                    "Request processing failed; nested exception is org.springframework.dao.EmptyResultDataAccessException: "
                            + "No class rs.edu.raf.si.bank2.mariadb.models.otc.User entity with id -1 exists!";
            String actualMessage = exception.getMessage();
            assertEquals(expectedMessage, actualMessage);
        } catch (Exception e) {
            fail(e.getMessage());
